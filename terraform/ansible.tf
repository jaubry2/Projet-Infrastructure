...
${node.name} ansible_host=${
    ...
    length(node.network_interface[0].access_config) > 0 ? "${node.network_interface[0].access_config[0].nat_ip} ansible_host=${google_compute_instance.nodes["edge"].network_interface[0].access_config[0].nat_ip} ansible_user=${var.ssh_user} ansible_ssh_private_key_file=${replace(var.ssh_pub_key, ".pub", "")}" : "${node.network_interface[0].network_ip} ansible_ssh_common_args='-o ProxyJump=terraform-user@${google_compute_instance.nodes["master"].network_interface[0].access_config[0].nat_ip}' ansible_host=${google_compute_instance.nodes["edge"].network_interface[0].access_config[0].nat_ip} ansible_user=${var.ssh_user} ansible_ssh_private_key_file=${replace(var.ssh_pub_key, ".pub", "")}"
}
%{ endif ~}
%{ endfor ~}

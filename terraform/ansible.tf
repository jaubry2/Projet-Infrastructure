...
[workers]
${google_compute_instance.nodes["worker"].name} ansible_host=${google_compute_instance.nodes["worker"].network_interface[0].access_config[0].nat_ip} ansible_user=${var.ssh_user} ansible_ssh_private_key_file=${replace(var.ssh_pub_key, ".pub", "")}
